...
async function executeSocial(bot, target, trust) {
    ...
    if (target === 'follow_trusted' || target === 'follow') {
        ...
        const sorted = [...players].sort((a, b) => a.entity.position.distanceTo(bot.entity.position) -
            b.entity.position.distanceTo(bot.entity.position));
        const chosen = sorted.find(p => !trust.isThreat(p.username)) ?? sorted[0];
        if (!chosen?.entity)
            return { success: false, reason: 'no valid follow target' };
        bot.chat(`Following you, ${chosen.username}! 🏃`);
        logger_1.log.brain(`[social] Following ${chosen.username}`);
        ...
        while (Date.now() - start < FOLLOW_MS) {
            ...
            if (!bot.players[chosen.username]?.entity) {
                logger_1.log.brain(`[social] ${chosen.username} left, stopping follow`);
                break;
            }
    ...
    if (target.startsWith('follow:')) {
        ...
        if (!p?.entity)
            return { success: false, reason: `${name} not found` };
