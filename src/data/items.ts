...
const _mcDataCache: Record<string, any> = {};

export function getMcData(version: string): any {
...
export function hasAny(bot: Bot, names: string[]): boolean {
 ...
 */
export function getBestFood(bot: Bot): any | null {
  const mcData = getMcData(bot.version);

  // mcData.foods is a Record<id, {id, name, stackSize, saturation, foodPoints}>
  const foods: any[] = Object.values(mcData.foods as Record<string, any>)
    .sort((a: any, b: any) => b.saturation - a.saturation);
...
const TOOL_LIST: Record<string, string[]> = {
 ...
 */
export function getBestTool(bot: Bot, type: string): any | null {
 ...
 */
export function getBestPickaxeTier(bot: Bot): number {
  const inv = bot.inventory.items().map(i => i.name);
  if (inv.some(n => n === 'netherite_pickaxe' || n === 'diamond_pickaxe')) return 3;
  if (inv.some(n => n === 'iron_pickaxe'))   return 2;
  if (inv.some(n => n === 'stone_pickaxe'))  return 1;
  if (inv.some(n => n === 'wooden_pickaxe' || n === 'golden_pickaxe')) return 0;
  return -1;
}
 ...
 * -1 = no sword.
 */
export function getBestSwordTier(bot: Bot): number {
  const inv = bot.inventory.items().map(i => i.name);
  if (inv.some(n => n === 'netherite_sword' || n === 'diamond_sword')) return 3;
  if (inv.some(n => n === 'iron_sword'))   return 2;
  if (inv.some(n => n === 'stone_sword'))  return 1;
  if (inv.some(n => n === 'wooden_sword' || n === 'golden_sword')) return 0;
  return -1;
}
 ...
 */
export function canMineBlock(bot: Bot, blockName: string): boolean {
  const mcData = getMcData(bot.version);
  const block  = mcData.blocksByName[blockName];
  if (!block)               return false;   // unknown block
  if (!block.harvestTools)  return true;    // no tool required — hands are fine
 ...
 */
export function canHarvestBlock(bot: Bot, blockName: string): boolean {
  return canMineBlock(bot, blockName);
}

// ─── Armor helpers ─────────────────────────────────────────────────────────

const ARMOR_TIERS: Record<number, string[]> = {
  // slot 5 = head
 ...
 */
export function getBestArmorUpgrades(bot: Bot): { slot: number; item: any }[] {
  const upgrades: { slot: number; item: any }[] = [];
